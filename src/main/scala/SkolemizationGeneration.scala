...
object SkolemizationGeneration {
  ...
  def generateSkolemization(
    ...
    val (boundVariables, foundVariable) = findBoundVariablesUpToExistential(inputFormula, variableToSkolemize)
    if (foundVariable.isEmpty) {
      throw new ProofErrorException(s"Could not find existential variable $variableToSkolemize in formula for step $stepName")
    }
    ...
    writer.println(s"    pattern " + LeanPrettyPrinter.prettyLeanFOFFormula(foundVariable.get))
    ...
    writer.println(s"  let ⟨${renderSkolemFunctionName(skolemFunctionName)}, step_$resultStepName'⟩ := step_$stepName")
    LeanPrettyPrinter.variablesAsTemplates = previousState
    writer.println(s"  have step_$resultStepName : ${LeanPrettyPrinter.prettyLeanFOFFormula(outputFormula)} := by (first | exact step_$resultStepName' | clearExcept step_$resultStepName'; simp_all | clearExcept step_$resultStepName'; grind only)") 
  }

  def findBoundVariablesUpToExistential(formula: FOF.Formula, skolemizedVariable: String): (Set[String],Option[FOF.Formula]) = {
    formula match {
        case TPTP.FOF.AtomicFormula(f, args) => (Set.empty[String], None)
        case TPTP.FOF.QuantifiedFormula(q, variables, body) => {
            if(q == TPTP.FOF.!){
                val (boundVars, exists) = findBoundVariablesUpToExistential(body, skolemizedVariable)
        ...
        case TPTP.FOF.UnaryFormula(connective, body) => findBoundVariablesUpToExistential(body, skolemizedVariable)
        case TPTP.FOF.BinaryFormula(connective, left, right) => {
            val (leftVars, leftExists) = findBoundVariablesUpToExistential(left, skolemizedVariable)
            if(!leftExists.isEmpty){
                return (leftVars, leftExists)
            } else {
                val (rightVars, rightExists) = findBoundVariablesUpToExistential(right, skolemizedVariable)
  ...
  def checkSkolemizationDetailsAreConsistent(dag: ProofDag.Dag): Map[String, Int] = {
    ...
    dag.nodes.values.filter(node => AnnotationInformationHelpers.containsRuleStep("skolemize", node.additionalInfo)).foreach(node => {
        ...
        details.newSymbols.foreach { sym =>
            ...
            if(skolemFunctionsDefined.contains(sym) || skolemFunctionArities.contains(sym)){
                throw new ProofErrorException(s"Skolem function $sym is defined multiple times in the proof DAG")
            }
