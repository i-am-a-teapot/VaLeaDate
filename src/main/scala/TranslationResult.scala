...
object TranslationResult {
  ...
  def parseTranslationResult(
      res: JobScheduler.ProcessResult
  ): TranslationResult = {
    // if(res.exitCode != 0) {
    ...
    val variableDeclarations = sectionBetween(
      result,
      "% BEGIN Variable Declarations",
      "% END Variable Declarations"
    )
    val inputFormulas =
      sectionBetween(result, "% BEGIN Input Formulas", "% END Input Formulas\n")
