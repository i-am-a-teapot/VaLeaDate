...
set_option maxRecDepth 100000000""")
  ...
  private def writeLeanProofSteps(
   ...
   = {
    ...
    topologicalSort.foreach { nodeName =>
      ...
      if (
        !(node.role == "axiom" || (containsConjectures && node.role == "conjecture") || (!containsConjectures && node.role == "negated_conjecture") || AnnotationInformationHelpers
          .isCth(node.additionalInfo))
      ) {
        if (
          ...
          val parentFormula = parent.formula.formula match {
            ...
            case _                         =>
              throw new ProofErrorException(
                s"Expected FOF formula for parent of skolemization step $nodeName"
              )
          }
          val skolemizedVariable =
            skolemizationDetails.skolemDefinitions.headOption
              .map(_._1)
              .getOrElse {
                throw new ProofErrorException(
                  s"No skolemized variable found in details for node $nodeName"
                )
              }
          val skolemFunctionName =
            skolemizationDetails.newSymbols.headOption.getOrElse {
              throw new ProofErrorException(
          ...
          if (!node.formula.isInstanceOf[TPTP.FOFAnnotated]) {
            throw new ProofUnsureException(
              s"Expected FOF formula for skolemization step $nodeName, found ${node.formula.getClass}"
            )
          }
          generateSkolemization(
            writer,
            parent.name,
            parentFormula,
            skolemizedVariable,
  ...
  private def writeLeanSources(
    ...
    sources.foreach { sourceName =>
      writer.print(
        LeanPrettyPrinter.prettyLeanSyntax(dag.nodes(sourceName).formula)
      )
      writer.print(" → ")
    }
  }
  ...
  private def writeLeanConjecture(
    ...
    if (dag.conjectures.length > 1) {
      throw new ProofUnsureException(
        s"Multiple conjecture nodes found in DAG: ${dag.conjectures.mkString(", ")}. Only one conjecture node is supported."
      )
    }
    if (dag.conjectures.isEmpty) {
      ...
      dag.conjectures.foreach { conjectureName =>
        writer.print(
          LeanPrettyPrinter.prettyLeanSyntax(dag.nodes(conjectureName).formula)
        )
      }
    }
  }
  ...
  def writeFullProof(
    ...
    var sourcesToPrint = dag.sources.filter(sourceName =>
      dag.nodes(sourceName).role == "axiom" || (!containsConjectures && dag
        .nodes(sourceName)
    ...
    writer.println(" := by")
    writer.print("  intro ")
    sourcesToPrint.foreach { sourceName =>
