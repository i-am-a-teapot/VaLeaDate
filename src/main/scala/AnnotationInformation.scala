...
final case class SkolemizationInformation(
    newSymbols: Seq[String],
    skolemDefinitions: Seq[(String, String, Seq[String])]
) extends AdditionalInformation {
  override def toString(): String = {
    val newSymbolsStr = newSymbols.mkString(", ")
    val skolemDefsStr = skolemDefinitions
      .map { case (v, f, args) =>
        s"($v, $f, [${args.mkString(", ")}])"
      }
      .mkString(", ")
...
final case class UnnamedInformation(inferenceInformation: InferenceInformation)
    extends ParentInformation
...
final case class InferenceInformation(
    status: String,
    rule: String,
    parents: Seq[ParentInformation],
    additionalInfo: Seq[AdditionalInformation] = Seq.empty
) extends AnnotationInformation {
  override def toString(): String = {
    val parentsStr = parents
      .map {
...
final case class FileInformation(fileName: String, formulaName: String)
    extends AnnotationInformation {
  override def toString(): String =
    s"FileInformation(fName=$fileName, name=$formulaName)"
}
...
final object AnnotationInformationHelpers {
  ...
  def getSkolemizationInformation(
      annotationInformation: AnnotationInformation
  ): SkolemizationInformation = {
    ...
    if (skolemizeAnnotations.length != 1) {
      throw new ProofErrorException(
        s"Expected exactly one skolemization step, found ${skolemizeAnnotations.length}"
      )
    }
    ...
    val details = skolemizationDetails.head match {
      case info @ SkolemizationInformation(newSymbols, skolemDefinitions) =>
        info
      case _ =>
        throw new ProofErrorException(
          s"Expected skolemization details, found ${skolemizationDetails.head.getClass}"
        )
    }
    return details
  }

  private def inferencesFromTerm(
      term: TPTP.GeneralTerm
  ): Option[InferenceInformation] = {
    if (term.list.isDefined) None
    else {
      term.data match {
        case Seq(TPTP.MetaFunctionData("inference", args))
            if args.size >= 3 => {
          val statusArg = args(1)
          val status = gatherKeywordsInTerm(statusArg, Set("status"))
            .flatMap { metaFunctionData =>
          ...
          if (rule == "skolemize") {
            Logger.println(
              s"Extracting skolemization details for term: ${term.pretty}",
              Logger.VERBOSITY_MEDIUM
            )
            details = details :+ extractSkolemizationDetails(statusArg)
          }
          Logger.println(
            s"Status for inference: $status",
            Logger.VERBOSITY_MEDIUM
          )
            ...
            if (argument.list.isDefined) {
              for (arg <- argument.list.get) {
                ...
                if (parentInference.nonEmpty) {
                  parentInformation = parentInformation ++ Seq(
                    UnnamedInformation(parentInference.get)
                  )
                } else {
                  ...
                  if (name.nonEmpty) {
                    parentInformation =
                      parentInformation :+ NamedParentInformation(name.get)
                  }
                }
              }
            }
  ...
  def getInformationFromAnnotation(
    ...
    if (fileInfo.isDefined) {
      ...
      if (fileNameOpt.isDefined && fileInputFormulaNameOpt.isDefined) {
        Logger.println(
          s"File name: ${fileNameOpt.get}",
          Logger.VERBOSITY_MEDIUM
        )
        Logger.println(
          s"File input formula name: ${fileInputFormulaNameOpt.get}",
          Logger.VERBOSITY_MEDIUM
        )
        return Some(
          FileInformation(fileNameOpt.get, fileInputFormulaNameOpt.get)
        )
      }
    }
  ...
  def extractSkolemizationDetails(
      inputStep: TPTP.GeneralTerm
  ): SkolemizationInformation = {
    ...
    val newSymbolTerm = newSymbols.headOption.getOrElse {
      throw new ProofErrorException(
        s"No new_symbols term found in skolemization details: ${inputStep.pretty}"
      )
    }
    val newSymbolInformation = newSymbolTerm match {
      case MetaFunctionData(f, args) => {
        ...
        Logger.println(
          s"Type of new symbols term: ${typeOfNewSymobls}",
          Logger.VERBOSITY_MEDIUM
        )
        val newSymbols = args(1).list match {
          ...
          case None =>
            throw new ProofUnsureException(
              s"Expected new_symbols term to have a list as the second argument, found ${args(1).pretty}"
            )
        }
        Logger.println(
          s"New symbols term: ${newSymbols}",
          Logger.VERBOSITY_MEDIUM
        )
        newSymbols
      }
      case _ =>
        throw new ProofUnsureException(
          s"Expected new_symbols term to be a MetaFunctionData, found ${newSymbolTerm.pretty}"
        )
    }

    val skolemizeTerm = skolemizeTerms.headOption.getOrElse {
      throw new ProofErrorException(
        s"No skolemize term found in skolemization details: ${inputStep.pretty}"
      )
    }
    val skolemArityInformation = skolemizeTerm match {
      case MetaFunctionData(f, args) => {
        ...
        if (args.size < 2) {
          throw new ProofErrorException(
            s"Expected skolemize term to have at least 2 arguments, found ${args.size}"
          )
        }
        val variableToSkolemize = args(0).data match {
          ...
          case _                         =>
            throw new ProofErrorException(
              s"Expected skolemize term to have a variable as the first argument, found ${args(0).pretty}"
            )
        }
        Logger.println(
          s"Variable to skolemize: ${variableToSkolemize}",
          Logger.VERBOSITY_MEDIUM
        )
        val skolemFunctionSize = args(1).data match {
          ...
          case _ =>
            throw new ProofErrorException(
              s"Expected skolemize term to have a function as the second argument, found ${args(1).pretty}"
            )
        }
        Logger.println(
          s"Skolem function: ${skolemFunctionSize._1}(${skolemFunctionSize._2.mkString(", ")})",
          Logger.VERBOSITY_MEDIUM
        )
      ...
      case _ =>
        throw new ProofUnsureException(
          s"Expected skolemize term to be a MetaFunctionData, found ${skolemizeTerm.pretty}"
        )
    }
  ...
  private def inferenceStatus(
      inference: TPTP.MetaFunctionData
  ): Option[String] = {
    inference match {
  ...
  private def inferenceRule(
      inference: Option[TPTP.MetaFunctionData]
  ): Option[String] = {
    inference match {
      case Some(TPTP.MetaFunctionData(_, inferenceData))
          if inferenceData.size >= 1 => {
  ...
  private def fileName(
      fileInfo: Option[TPTP.MetaFunctionData]
  ): Option[String] = {
    fileInfo match {
  ...
  private def fileInputFormulaName(
      fileInfo: Option[TPTP.MetaFunctionData]
  ): Option[String] = {
    fileInfo match {
      case Some(TPTP.MetaFunctionData("file", args)) if args.size >= 2 =>
        args(1).data.toSeq.collectFirst { case MetaFunctionData(role, Seq()) =>
          stripOuterQuotes(role)
        }
      case _ => None
    }
  }
  ...
  private def stripOuterQuotes(value: String): String = {
    if (
      (value.startsWith("'") && value
        .endsWith("'")) || (value.startsWith("\"") && value.endsWith("\""))
    ) {
