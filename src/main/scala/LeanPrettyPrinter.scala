...
object LeanPrettyPrinter {
  ...
  def prettyLeanSyntax(value: TPTP.AnnotatedFormula): String = value match {
    ...
    case _                                   =>
      throw new ProofUnsureException(
        s"Unsupported annotated formula type: ${value.getClass}"
      )
  }

  def prettyLeanFOFStatement(statement: TPTP.FOF.Statement): String =
    statement match {
  ...
  def prettyLeanFOFFormula(formula: TPTP.FOF.Formula): String = formula match {
    case TPTP.FOF.AtomicFormula(f, args) =>
      prettyLeanApp(f, args.map(prettyLeanFOFTerm))
    ...
    case TPTP.FOF.Equality(left, right) =>
      s"(${prettyLeanFOFTerm(left)} = ${prettyLeanFOFTerm(right)})"
    case TPTP.FOF.Inequality(left, right) =>
      s"(${prettyLeanFOFTerm(left)} ≠ ${prettyLeanFOFTerm(right)})"
  }

  private def prettyLeanCNFStatement(statement: TPTP.CNF.Statement): String =
    statement match {
      case TPTP.CNF.Logical(formula) => prettyLeanCNFClause(formula)
    }

  private def prettyLeanCNFClause(clause: TPTP.CNF.Formula): String =
    clause.map(prettyLeanCNFLiteral).mkString(" ∨ ")

  private def prettyLeanCNFLiteral(literal: TPTP.CNF.Literal): String =
    literal match {
      case TPTP.CNF.PositiveAtomic(formula) =>
        prettyLeanCNFAtomicFormula(formula)
      case TPTP.CNF.NegativeAtomic(formula) =>
        s"¬ ${prettyLeanCNFAtomicFormula(formula)}"
      case TPTP.CNF.Equality(left, right) =>
        s"(${prettyLeanCNFTerm(left)} = ${prettyLeanCNFTerm(right)})"
      case TPTP.CNF.Inequality(left, right) =>
        s"(${prettyLeanCNFTerm(left)} ≠ ${prettyLeanCNFTerm(right)})"
    }

  private def prettyLeanCNFAtomicFormula(
      formula: TPTP.CNF.AtomicFormula
  ): String =
  ...
  private def prettyLeanFOFTerm(term: TPTP.FOF.Term): String = term match {
    case TPTP.FOF.AtomicTerm(f, args) =>
      prettyLeanApp(f, args.map(prettyLeanFOFTerm))
    case TPTP.FOF.Variable(name)       => leanVarIdent(name)
  ...
  private def prettyLeanCNFTerm(term: TPTP.CNF.Term): String = term match {
    case TPTP.CNF.AtomicTerm(f, args) =>
      prettyLeanApp(f, args.map(prettyLeanCNFTerm))
    case TPTP.CNF.Variable(name)       => leanVarIdent(name)
  ...
  private def prettyLeanUntypedVariable(name: String): String =
    if (variablesAsTemplates) s"_" else s"(${name} : ι)"

  private def prettyLeanFOFQuantifier(quantifier: TPTP.FOF.Quantifier): String =
    quantifier match {
      case TPTP.FOF.! => "∀"
      case TPTP.FOF.? => "∃"
      case _          =>
        throw new ProofUnsureException(
          s"Unsupported quantifier in FOF: $quantifier"
        )
    }
  ...
  private def prettyLeanFOFBinary(
      connective: TPTP.FOF.BinaryConnective
  ): String = connective match {
    case TPTP.FOF.<=>  => "↔"
    case TPTP.FOF.Impl => "→"
    case TPTP.FOF.<=   => "←"
    case TPTP.FOF.|    => "∨"
  ...
  private def leanVarIdent(name: String): String = {
