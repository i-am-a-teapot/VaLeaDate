...
object JobScheduler {
  ...
  final case class ProcessResult(
      ...
      usedParents: Seq[String] = Seq.empty
  )
  ...
  private def runProcess(spec: JobSpec): ProcessResult = {
    ...
    try {
      ...
      val timedOut = spec.timeout match {
        case Some(dur) =>
          !p.waitFor(dur.toMillis, java.util.concurrent.TimeUnit.MILLISECONDS)
        case None =>
          try { p.waitFor(); false }
      ...
      try Files.deleteIfExists(tIn)
      catch { case _: Throwable => () }
      try Files.deleteIfExists(tOut)
      catch { case _: Throwable => () }
      try Files.deleteIfExists(tErr)
      catch { case _: Throwable => () }
    }
  }

  /** Run jobs for the given nodes in parallel. The `jobBuilder` receives the
    * node and its parents. Returns a Future completing with a map from node
    * name -> ProcessResult when all specified nodes are done.
    */
  def runNodes(inferencesToRun: Seq[TPTPProblemGenerator.Inference])(
      jobBuilder: (TPTPProblemGenerator.Inference) => JobSpec
  )(implicit ec: ExecutionContext): Future[Map[String, ProcessResult]] = {
    val jobFutures = inferencesToRun.map { inference =>
      Future {
        ...
        val usedParents = res.stdout.linesIterator
          .filter(_.startsWith("-- VaLeaDATE info:"))
          .flatMap { line =>
            line
              .stripPrefix("-- VaLeaDATE info:")
              .trim
              .split("\\s+")
              .filter(_.nonEmpty)
          }
        ...
        (
          inference.name,
          res.copy(usedParents =
            usedParents.map(parentNo =>
              AnnotatedFormulaHelpers.sanitizeName(
                inference.premises(parentNo.toInt - 1).name
              )
            )
          )
        )
  ...
  def runFuture(
      spec: JobSpec
  )(implicit ec: ExecutionContext): Future[ProcessResult] = Future {
    runProcess(spec)
  }

  def runFutures(specs: Seq[JobSpec])(implicit
      ec: ExecutionContext
  ): Future[Seq[ProcessResult]] = Future.sequence(specs.map(runFuture))
