...
object  ProofDag {
  ...
  final case class Dag(nodes: Map[String, Node]) {
    ...
    lazy val sources: Seq[String] = {
      val indeg = scala.collection.mutable.Map.empty[String, Int].withDefaultValue(0)
    ...
    lazy val sinks: Seq[String] = {
      val outdeg = scala.collection.mutable.Map.empty[String, Int].withDefaultValue(0)
    ...
    lazy val conjectures: Seq[String] = nodes.values.toSeq.filter(_.role == "conjecture").map(_.name).sorted
    lazy val negatedConjectures: Seq[String] = nodes.values.toSeq.filter(_.role == "negated_conjecture").map(_.name).sorted
    lazy val countersatisfiable: Seq[String] = nodes.values.toSeq.filter(node => AnnotationInformationHelpers.isCth(node.additionalInfo)).map(_.name).sorted
    lazy val axioms: Seq[String] = nodes.values.toSeq.filter(_.role == "axiom").map(_.name).sorted
    ...
    def toDot: String = {
      ...
      (Seq("digraph ProofDag {", "  rankdir=BT;", "  node [fontname=\"Helvetica\"];", "  edge [arrowsize=0.7];") ++
        nodeLines ++
        edgeLines ++
        Seq("}")).mkString("\n")
    }
  ...
  def fromProof(steps: Seq[TPTP.AnnotatedFormula], assumeThm: Boolean, negatedConjectureAsAxiom: Boolean): Dag = {
  ...
  private def stepName(step: TPTP.AnnotatedFormula): Option[String] = step match {
  ...
  private def stepRole(step: TPTP.AnnotatedFormula): Option[String] = step match {
